...
class LoanService(BaseService[Loan, LoanCreate, LoanUpdate]):
    ...
    def create_loan(
        ...
        *,
        ...
        # Déterminer la catégorie de l'utilisateur et ajuster les paramètres de l'emprunt
        if user.category == "vip":
            ...
            max_loans = 5
            loan_period_days = 14
        logger.info(f"Emprunt: user_id={user_id}, catégorie={user.category}, max_loans={max_loans}, loan_period_days={loan_period_days}")
    ...
    def notify_overdue_loans(self):
        """
        Envoie des notifications pour les emprunts en retard.
        """
        overdue_loans = self.loan_repository.get_overdue_loans()
        for loan in overdue_loans:
            user = self.user_repository.get(id=loan.user_id)
            logger.warning(f"Notification envoyée à {user.email} pour l'emprunt en retard (livre {loan.book_id})")
            # Ici, ajoutez l'envoi d'email réel si besoin

    def reserve_book(self, user_id: int, book_id: int):
        logger.info(f"Réservation d'un livre: user_id={user_id}, book_id={book_id}")
