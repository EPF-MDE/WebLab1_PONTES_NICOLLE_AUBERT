...
def test_authenticate_user(db_session: Session):
    ...
    with pytest.raises(CustomException) as exc:
        service.authenticate(email="auth@example.com", password="wrongpassword")
    assert "Mot de passe incorrect" in str(exc.value)

    # Authentification échouée - email inexistant
    with pytest.raises(CustomException) as exc:
        service.authenticate(email="nonexistent@example.com", password="password123")
